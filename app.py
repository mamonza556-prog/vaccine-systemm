...
app = Flask(__name__)
app.secret_key = 'integrated_supabase_pro_render_final'
...
def add_child():
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute('INSERT INTO children (name, id_card, birth_date, address, visit_date) VALUES (%s,%s,%s,%s,%s)', 
                        (d.get('name'), d.get('id_card',''), d.get('birth_date'), d.get('address',''), datetime.now().strftime('%d/%m/%Y')))
    conn.close()
    return redirect(url_for('index'))
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute('UPDATE children SET weight=%s, height=%s, head_circ=%s, vaccines_today=%s, next_vaccine=%s, appoint_date=%s, visit_date=%s WHERE id=%s', 
                        (d.get('weight',0), d.get('height',0), d.get('head_circ',0), ", ".join(v_list), nv, nd, visit_date, id))
            ...
            for v in newly_added:
                ...
                v_short = v.split(' ')[0]
                cur.execute('INSERT INTO logs (date, vaccine_name, source_destination, pay, note) VALUES (%s,%s,%s,%s,%s)', 
                            (datetime.now().strftime('%Y-%m-%d'), v_short, "ให้บริการเด็ก", 1, f"ฉีดให้ {child['name']}"))
    conn.close()
    return redirect(url_for('index'))
...
def profile(id):
    ...
    pct = int((len([v for v in CORE_VACCINES if (child['vaccines_today'] or "") and v in child['vaccines_today']]) / len(CORE_VACCINES)) * 100)
...
<style>body{background:#f0fdf4; height:100vh; display:flex; align-items:center; justify-content:center; font-family:sans-serif;}.login-card{background:white; padding:40px; border-radius:40px; box-shadow:0 20px 50px rgba(0,0,0,0.1); width:100%; max-width:400px; text-align:center; border:none;}</style></head>
...
<div class="container px-4">
    <div class="d-flex justify-content-between align-items-center mb-3"><h5 class="fw-bold text-secondary m-0">ประวัติรายการ</h5><a href="/stock?view_deleted={{ 0 if view_deleted else 1 }}" class="btn btn-sm btn-outline-secondary rounded-pill px-3">{{ 'หน้าหลัก' if view_deleted else 'ดูถังขยะ' }}</a></div>
    {% for log in logs %}<div class="card border-0 shadow-sm rounded-4 p-3 mb-2">
    <div class="d-flex justify-content-between align-items-center"><div><b class="text-success">{{log.vaccine_name}}</b><br><small class="text-muted">{{log.date}} | {{log.source_destination}}</small><br><small class="text-secondary">{{log.note}}</small></div><div class="text-end fw-bold {{'text-success' if log.receive > 0 else 'text-danger'}} h5">{{'+' ~ log.receive if log.receive > 0 else '-' ~ log.pay}}</div></div></div>{% endfor %}
...
if __name__ == '__main__':
    # สำหรับ Render: ดึง Port จาก Environment
