        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            for v in newly_added:
                ...
                cur.execute('INSERT INTO logs (date, vaccine_name, source_destination, pay, note) VALUES (%s,%s,%s,%s,%s)', 
                            (datetime.now().strftime('%Y-%m-%d'), v_short, "ให้บริการเด็ก", 1, f"ฉีดให้น้อง {child['name']}"))
    conn.close()
    return redirect(url_for('index'))
...
<div class="container px-4 text-black">
    <div class="d-flex flex-column gap-3 mb-4">
        <div class="d-flex align-items-center gap-2">
            ...
            <a href="/stock?view_deleted={{ 0 if view_deleted else 1 }}" class="btn btn-sm {{ 'btn-outline-primary' if view_deleted else 'btn-outline-secondary' }} rounded-pill">{{ 'หน้าหลัก' if view_deleted else 'ดูถังขยะ' }}</a>
        </div>
