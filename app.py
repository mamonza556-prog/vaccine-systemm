...
app = Flask(__name__)
app.secret_key = 'integrated_supabase_pro_fixed'

# ⚠️ แก้ไขตรงนี้: เอา URI จาก Supabase มาใส่แทนอันนี้นะครับแก
DB_URL = "postgresql://postgres.dwykmsiqyhujvltslbwi:aniwatza11.@aws-1-ap-southeast-2.pooler.supabase.com:5432/postgres"

USER_LOGIN = "23256"
USER_PASS = "23256"

# ข้อมูลระบบ (ห้ามเปลี่ยน)
...
def init_db():
    # สร้างตารางบน Supabase (ถ้ายังไม่มี)
    conn = get_db()
    with conn:
        with conn.cursor() as cur:
            cur.execute('CREATE TABLE IF NOT EXISTS children (id SERIAL PRIMARY KEY, id_card TEXT, name TEXT, birth_date TEXT, address TEXT, weight REAL, height REAL, head_circ REAL, vaccines_today TEXT, next_vaccine TEXT, appoint_date TEXT, visit_date TEXT)')
            cur.execute('CREATE TABLE IF NOT EXISTS growth_history (id SERIAL PRIMARY KEY, child_id INTEGER, visit_date TEXT, age_at_visit TEXT, weight REAL, height REAL, head_circ REAL)')
            cur.execute('CREATE TABLE IF NOT EXISTS vaccine_records (id SERIAL PRIMARY KEY, child_id INTEGER, vaccine_name TEXT, receive_date TEXT, location TEXT)')
            cur.execute('CREATE TABLE IF NOT EXISTS logs (id SERIAL PRIMARY KEY, date TEXT, vaccine_name TEXT, source_destination TEXT, receive INTEGER DEFAULT 0, pay INTEGER DEFAULT 0, lot TEXT, exp TEXT, note TEXT, is_deleted INTEGER DEFAULT 0)')
    conn.close()
...
def index():
    ...
    for r in rows:
        ...
        d['coverage_pct'] = int((len([v for v in CORE_VACCINES if (d['vaccines_today'] or "") and v in d['vaccines_today']]) / len(CORE_VACCINES)) * 100)
    ...
    with conn:
        with conn.cursor() as cur:
            ...
            for v in newly_added:
                ...
                v_short = v.split(' ')[0]
                cur.execute('INSERT INTO logs (date, vaccine_name, source_destination, pay, note) VALUES (%s,%s,%s,%s,%s)', (datetime.now().strftime('%Y-%m-%d'), v_short, "ให้บริการเด็ก", 1, f"ฉีดให้ {child['name']}"))
    conn.close()
    return redirect(url_for('index'))
...
def profile(id):
    ...
    child = dict(c)
    child['age_str'] = calculate_age_be(child['birth_date'])
    child['coverage_pct'] = int((len([v for v in CORE_VACCINES if (child['vaccines_today'] or "") and v in child['vaccines_today']]) / len(CORE_VACCINES)) * 100)
    child['in_zone'] = any(z in (child['address'] or "") for z in ["ม.2", "ม.5", "ม.7", "หมู่ 2", "หมู่ 5", "หมู่ 7"])
    ...
    return render_template_string(PROFILE_HTML, child=child, history=hist, vaccines=VACCINES_ALL, v_dict=v_dict, today_be=today_be_str)
...
def stock():
    conn = get_db()
    cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute('SELECT SUM(receive) as rcv, SUM(pay) as py FROM logs WHERE date = %s AND is_deleted = 0', (datetime.now().strftime('%Y-%m-%d'),))
    stats = cur.fetchone()
    ...
    stock_map = {s['vaccine_name']: s['bal'] for s in bal_rows}
    cur.execute('SELECT * FROM logs WHERE is_deleted = 0 ORDER BY id DESC LIMIT 50')
    logs = cur.fetchall()
    conn.close()
    return render_template_string(STOCK_HTML, logs=logs, stock_map=stock_map, vaccine_names=STOCK_VACCINES, sources=STOCK_SOURCES, stats=stats, today_date=datetime.now().strftime('%Y-%m-%d'), note_choices=STOCK_NOTES)
...
def add_stock():
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute('INSERT INTO logs (date, vaccine_name, source_destination, receive, pay, lot, exp, note) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)', (d['log_date'], d['vaccine_name'], d['source_destination'], rcv, py, d['lot'], d['exp_date'], note))
    conn.close()
    return redirect(url_for('stock'))

# --- หน้าจอเสริม (ลบ / Export) ---
...
def delete_child(id):
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute('DELETE FROM children WHERE id=%s', (id,))
            cur.execute('DELETE FROM growth_history WHERE child_id=%s', (id,))
            cur.execute('DELETE FROM vaccine_records WHERE child_id=%s', (id,))
    conn.close()
    return redirect(url_for('index'))
...
@app.route('/delete_v_record/<int:record_id>/<int:child_id>', methods=['POST'])
@login_required
def delete_v_record(record_id, child_id):
    conn = get_db()
    with conn:
        with conn.cursor() as cur: cur.execute('DELETE FROM vaccine_records WHERE id=%s', (record_id,))
    conn.close()
    return redirect(url_for('profile', id=child_id))
...
<style>
    ...
    .nav-custom { background:#064e3b; color:white; padding:15px; border-radius:0 0 30px 30px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); }
...
<nav class="navbar navbar-dark bg-success sticky-top shadow-sm">
    <div class="container-fluid">
        ...
        <span class="navbar-brand fw-bold">รพ.สต.บางตะเคียน (Supabase)</span>
        <div class="offcanvas offcanvas-start bg-success text-white" id="sidebar" style="width:280px">
            ...
            <div class="offcanvas-body p-0"><div class="list-group list-group-flush mt-3"><a href="/" class="list-group-item list-group-item-action bg-transparent text-white border-0 py-3 fs-5"><i class="fas fa-child me-3"></i>ทะเบียนเด็ก / วัคซีน</a><a href="/stock" class="list-group-item list-group-item-action bg-transparent text-white border-0 py-3 fs-5"><i class="fas fa-box-open me-3"></i>คลังวัคซีน (Stock)</a><hr class="mx-3 opacity-25"><a href="/logout" class="list-group-item list-group-item-action bg-transparent text-white-50 border-0 py-3"><i class="fas fa-sign-out-alt me-3"></i>ออกจากระบบ</a></div></div>
        </div>
    </div>
</nav>
'''

# --- Templates (ความสวยเดิม 100%) ---
LOGIN_HTML = '''
<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1">''' + STYLE_ASSETS + '''
<style>body{background:#f0fdf4; height:100vh; display:flex; align-items:center; justify-content:center;}.login-card{background:white; padding:40px; border-radius:40px; box-shadow:0 20px 50px rgba(0,0,0,0.1); width:100%; max-width:400px; text-align:center; border:none;}</style></head>
<body><div class="login-card"><h2 class="fw-bold text-success mb-4">ยินดีต้อนรับ</h2><form method="POST"><input type="text" name="username" class="form-control mb-3 py-3 rounded-pill text-center border-0 bg-light" placeholder="User" required><input type="password" name="password" class="form-control mb-4 py-3 rounded-pill text-center border-0 bg-light" placeholder="Pass" required><button class="btn btn-success w-100 py-3 rounded-pill fw-bold">เข้าสู่ระบบ</button></form></div></body></html>
'''
...
<div class="container mt-4">
    <form class="mb-4"><input type="text" name="search" class="form-control rounded-pill mb-3 border-0 shadow-sm py-2" placeholder="ค้นชื่อ หรือ เลขบัตร..." value="{{search}}">
    <div class="d-flex justify-content-center gap-2">
        <button name="filter" value="all" class="btn btn-sm rounded-pill px-4 {{'btn-dark' if current_filter=='all' else 'btn-outline-dark'}}">ทั้งหมด</button>
        <button name="filter" value="this_month" class="btn btn-sm rounded-pill px-4 {{'btn-warning text-dark' if current_filter=='this_month' else 'btn-outline-warning text-dark'}}">เดือนนี้</button>
        <button name="filter" value="overdue" class="btn btn-sm rounded-pill px-4 {{'btn-danger' if current_filter=='overdue' else 'btn-outline-danger'}}">ขาดนัด</button>
    </div></form>
    {% for c in children %}<div class="card card-child p-3"><div class="row align-items-center"><div class="col-8"><div class="mb-1"><span class="badge badge-zone {{ 'bg-in' if c.in_zone else 'bg-out' }}">{{ 'ในเขต' if c.in_zone else 'นอกเขต' }}</span>{% if c.status=='overdue' %}<span class="badge bg-danger ms-1 animate-pulse">ขาดนัด!</span>{% elif c.status=='this_month' %}<span class="badge bg-warning text-dark ms-1">คิวเดือนนี้</span>{% endif %}</div><h5 class="fw-bold m-0 text-dark">{{c.name}}</h5><small class="text-muted">📅 {{c.birth_date}} ({{c.age_str}})</small><div class="mt-2"><div class="d-flex justify-content-between align-items-center"><small class="text-muted" style="font-size:0.7rem">ครอบคลุม</small><small class="fw-bold text-success" style="font-size:0.7rem">{{c.coverage_pct}}%</small></div><div class="progress-tiny"><div class="progress-bar bg-success" style="width:{{c.coverage_pct}}%"></div></div></div><div class="mt-3 d-flex gap-1"><button class="btn btn-sm btn-success rounded-pill px-3 shadow-sm" data-bs-toggle="modal" data-bs-target="#service{{c.id}}"><i class="fas fa-syringe me-1"></i>ให้บริการ</button><a href="/profile/{{c.id}}" class="btn btn-sm btn-outline-primary rounded-pill px-3 ms-1"><i class="fas fa-book-medical me-1"></i>ประวัติ</a><form action="/delete_child/{{c.id}}" method="POST" class="m-0" onsubmit="return confirm('ลบข้อมูล?')"><button class="btn btn-sm text-danger border-0 p-1"><i class="fas fa-trash-alt"></i></button></form></div></div><div class="col-4 text-end border-start small text-muted">นัดหน้า<div class="fw-bold text-primary fs-6">{{c.appoint_date or '-'}}</div><div class="text-danger fw-bold small">{{c.next_vaccine or ''}}</div></div></div></div>
...
<style>.header-bg-stock { background: linear-gradient(135deg, #064e3b, #059669); color: white; padding: 30px 20px; border-radius: 0 0 40px 40px; }.stock-card { min-width:110px; background:rgba(255,255,255,0.15); backdrop-filter:blur(10px); padding:15px; border-radius:20px; text-align:center; color:white; border:1px solid rgba(255,255,255,0.2); }</style></head>
<body>''' + NAV_BAR + '''
<div class="header-bg-stock mb-4"><div class="container d-flex justify-content-between px-0"><div><h4 class="fw-bold m-0">คลังวัคซีน</h4><small class="opacity-75">Supabase Online</small></div><a href="/export_stock" class="btn btn-sm btn-light text-success rounded-pill fw-bold">📊 Excel</a></div><div class="d-flex overflow-auto gap-3 mt-3 pb-2" style="scrollbar-width:none">{% for name in vaccine_names %}<div class="stock-card"><small class="opacity-75">{{name}}</small><div class="h5 fw-bold">{{stock_map.get(name, 0)}}</div></div>{% endfor %}</div></div>
<div class="container px-3"><div class="d-flex justify-content-between align-items-center mb-3"><h5 class="fw-bold text-secondary m-0">{{ '🗑️ ถังขยะ' if view_deleted else '📑 ประวัติรายการ' }}</h5><a href="/stock?view_deleted={{ 0 if view_deleted else 1 }}" class="btn btn-sm btn-outline-secondary rounded-pill px-3">{{ 'หน้าหลัก' if view_deleted else 'ดูถังขยะ' }}</a></div>
{% for log in logs %}<div class="card border-0 shadow-sm rounded-4 p-3 mb-2"><div class="d-flex justify-content-between align-items-center"><div><b class="text-success">{{log.vaccine_name}}</b><br><small class="text-muted">{{log.date}} | {{log.source_destination}}</small><br><small class="text-secondary">{{log.note}}</small></div><div class="text-end fw-bold {{'text-success' if log.receive > 0 else 'text-danger'}} h5">{{'+' ~ log.receive if log.receive > 0 else '-' ~ log.pay}}</div></div></div>{% endfor %}
<button class="btn btn-success btn-float fw-bold shadow-lg" data-bs-toggle="modal" data-bs-target="#stockModal">+ บันทึกรับ/จ่าย</button></div>
<div class="modal fade" id="stockModal" tabindex="-1"><div class="modal-dialog modal-dialog-centered text-black"><div class="modal-content p-4 rounded-4 border-0"><h5>บันทึกคลัง</h5><form action="/add_stock" method="POST" class="row g-3"><div class="col-12">วันที่<input type="date" name="log_date" class="form-control" required value="{{ today_date }}"></div><div class="col-12">ชื่อวัคซีน<select name="vaccine_name" class="form-select">{% for n in vaccine_names %}<option>{{n}}</option>{% endfor %}</select></div><div class="col-6">ประเภท<select name="action" class="form-select"><option value="receive">รับเข้า</option><option value="pay">จ่ายออก</option></select></div><div class="col-6">จำนวน<input type="number" name="amount" class="form-control" required></div><div class="col-12">หน่วยงาน<select name="source_destination" class="form-select">{% for s in sources %}<option>{{s}}</option>{% endfor %}</select></div><div class="col-6">Lot<input type="text" name="lot" class="form-control"></div><div class="col-6">Exp<input type="date" name="exp_date" class="form-control"></div><button class="btn btn-success w-100 rounded-pill py-3 mt-3 fw-bold">ยืนยัน</button></form></div></div></div>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script></body></html>
'''

PROFILE_HTML = '''
<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1">''' + STYLE_ASSETS + '''
<style>.header-pink{background: linear-gradient(135deg, #ec4899 0%, #f43f5e 100%); color:white; padding:20px; border-radius:0 0 30px 30px;}.info-card{background:white; border-radius:20px; padding:20px; margin-bottom:20px; box-shadow:0 5px 15px rgba(0,0,0,0.03)}</style></head>
...
<div class="info-card border-top border-4 border-primary"><h5>บันทึกเจริญเติบโต</h5><table class="table table-sm small text-center"><thead><tr><th>วันที่</th><th>อายุ</th><th>หนัก</th><th>สูง</th><th>ศีรษะ</th><th>ลบ</th></tr></thead><tbody>{% for h in history %}<tr><td>{{h.visit_date}}</td><td>{{h.age_at_visit}}</td><td>{{h.weight}}</td><td>{{h.height}}</td><td>{{h.head_circ}}</td><td><form action="/delete_growth/{{h.id}}/{{child.id}}" method="POST"><button class="btn btn-sm text-danger border-0 p-0"><i class="fas fa-trash-alt"></i></button></form></td></tr>{% endfor %}</tbody></table></div>
<div class="info-card border-top border-4 border-success"><h5>สถานะวัคซีน ({{child.coverage_pct}}%)</h5>{% for v in vaccines %}<div class="d-flex justify-content-between align-items-center border-bottom py-2 small"><span>{{v}}</span>{% if v in v_dict %}<div class="text-end text-success"><b>✅ {{v_dict[v].receive_date}}</b><br><small class="text-muted"><i class="fas fa-hospital"></i> {{v_dict[v].location}}</small></div>{% else %}<span class="text-muted">⏳ ยังไม่ได้รับ</span>{% endif %}</div>{% endfor %}</div></div>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script></body></html>
'''
